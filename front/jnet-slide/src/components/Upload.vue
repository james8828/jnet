...
const uploadChunks = async (uploadedList: string[] = []) => {
  const requestList = data.value.filter(({ hash }) => !uploadedList.includes(hash))
      .map(({ chunk, index, fileHash, size }) => {
        ...
        // 切片数量
        formData.append('chunks', data.value.length)
        // 切片文件hash
        formData.append('md5', fileHash)
        // 大文件的文件名
        formData.append('name', container.file.name)
        // 大文件hash
        //formData.append('fileHash', container.hash)
        return { formData, index }
      })
  debugger
  const arr = requestList.map(async ({ formData, index }) =>
          bigUploadRequest({
            url: '/jnet/image/attachment/uploadChunk',
            method: 'post',
            data: formData,

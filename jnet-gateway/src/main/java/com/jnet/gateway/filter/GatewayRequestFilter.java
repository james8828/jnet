...
import com.jnet.gateway.constants.Constants;
import com.jnet.gateway.configure.ArtGatewayProperties;
import com.jnet.gateway.util.WebFluxUtil;
...
public class GatewayRequestFilter implements GlobalFilter, Ordered {
	...
	public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
		...
		ServerHttpRequest build = request.mutate()
			.header(Constants.GATEWAY_TOKEN_HEADER,
					new String(Base64.getEncoder().encode((Constants.GATEWAY_TOKEN_VALUE).getBytes())))
			.build();
	...
	private Mono<Void> checkForbidUri(ServerHttpRequest request, ServerHttpResponse response) {
		...
		if (!shouldForward) {
			return WebFluxUtil.webFluxResponseWriter(response, R.fail("该URI不允许外部访问"),

...
public class RTMOPL : ServiceBase
{
	private Container components;

	public static int MaxReadBuffSize = 32768;

	public static Buffers m_OPLBytesBuffers = null;
	...
	public bool m_bTestNameColumn;

	public bool m_bMethodsTable;
	...
	public RTMOPL()
	{
		...
		m_NNBase.NNBaseOpen(bLogging: true, "RTMOPL", "RTMOPL", "OPL");
		string DBFile = "";
	...
	private bool GetDBDataForService()
	{
		...
		try
		{
			if (bDBAvailable = m_NNBase.OpenDBConnection(ref myConnection, ref myCommand, 7))
			{
				...
				m_NNBase.log("RTMOPL version = " + version + "  Database version = " + db_ver, isXml: false, "RTMOPL");
				m_NNBase.CheckIfAuthorized("RTMOPL", ref myCommand, ShutDown);
				m_NNBase.CheckAssemblyVersion("RTMOPL", version, ref myCommand, ShutDown);
				m_NNBase.RegisterStart(myCommand);
				bDBAvailable = m_NNBase.bDBAvailable;
				if (bDBAvailable)
				{
					sql = "select tot_messages_processed from dba.health_ping where process_name = 'RTMOPL' and host = '" + m_NNBase.GetLocalPOP() + "'";
					...
					theReader.Close();
					sql = "select column_name from sys.syscolumn where table_id in (select table_id from sys.systable where table_name = 'operator_privilege') order by column_id";
					myCommand.CommandText = sql;
					...
					while (theReader.Read())
					{
						if (!theReader.IsDBNull(0))
						{
							...
							if (Comp.Compare(field, "test_name", CompOpt) == 0)
							{
								m_bTestNameColumn = true;
							}
						}
					}
					theReader.Close();
					...
					while (theReader.Read())
					{
						if (!theReader.IsDBNull(0))
						{
							...
							if (string.Compare(field2, "inst_class", ignoreCase: true) == 0)
							{
								m_b_loc_last_update_inst_class_column = true;
							}
							else if (string.Compare(field2, "inst_type", ignoreCase: true) == 0)
							{
					...
					while (theReader.Read())
					{
						if (!theReader.IsDBNull(0))
						{
							...
							if (Comp.Compare(table, "methods", CompOpt) == 0)
							{
								m_bMethodsTable = true;
							}
						}
					}
					theReader.Close();
		...
		{
			handleDBException(e, "initializing", "RTMOPL");
		}
		catch (Exception e2)
		{
			handleException(e2, "initializing", "RTMOPL");
		}
		finally
		{
			myCommand?.Dispose();
	...
	protected override void Dispose(bool disposing)
	{
		if (disposing)
		{
			...
			{
				m_NNBase.log("Disposing", isXml: false, "RTMOPL");
			}
			m_OneMinTimer.Close();
		...
		try
		{
			if (GetDBConnByTimer())
			{
				...
				else if (bStopLogging)
				{
					...
					m_NNBase.log("logging turned off", isXml: false, "RTMOPL");
			...
			if (bWorkerThreadsWereStarted && !m_bShuttingDown && (m_Port == null || m_Port.m_CommType == null || !m_Port.m_CommType.IsAlive()))
			{
				...
				m_NNBase.log("Shutting down due to the death of worker thread", isXml: false, "RTMOPL");
				ShutDown(bExit: true);
			}
		}
		catch (OdbcException e)
			...
			if (GetDBConnByTimer())
			{
				...
				if (theReader.Read())
				{
					...
					if (!theReader.IsDBNull(20))
					{
						...
						{
							m_NNBase.ReportErrorDB("System times of Database host and RTMOPL host exceed one minue", "E", "initializing ports", "InitializePorts", "");
							m_NNBase.m_Status = 0;
						}
					}
					if (par.used == "T")
	...
	private void InitializePort(PortParams par)
	{
		try
		{
			...
			if (!found && par.portActive == 1)
			{
				...
				if (m_NNBase.m_isLogging)
				{
					m_NNBase.log("Add Port " + ((par.connectRemote > 0) ? par.remotePort : par.portNum) + " " + par.commProtocol + " C" + par.connectRemote + " M" + par.multiConnect + " T" + par.portType, isXml: false, "RTMOPL");
				}
	...
	private void ShutDown(bool bExit)
	{
		...
		if (m_NNBase.m_isLogging)
		{
			m_NNBase.log("Shutting down", isXml: false, "RTMOPL");
		}
		m_OneMinTimer.Stop();
		m_OneMinTimer.Close();
		if (m_NNBase.m_isLogging)
		{
			m_NNBase.log("Timer closed", isXml: false, "RTMOPL");
		}
		CloseListenerPorts();
		foreach (Port p in m_portsList)
		{
			if (p.m_par.connectRemote > 0)
			{
				...
				if (m_NNBase.m_isLogging)
				{
					m_NNBase.log("Closing client port " + sport, isXml: false, "RTMOPL");
				}
				p.m_CommType.Notify(-1);
				if (m_NNBase.m_isLogging)
				{
					m_NNBase.log("Closed client port " + sport, isXml: false, "RTMOPL");
				}
			}
		}
		m_portsList.Clear();
		if (m_NNBase.m_isLogging)
		{
			m_NNBase.log("Shutdown", isXml: false, "RTMOPL");
			m_NNBase.StopLogging();
		}
		if (m_OPLBytesBuffers != null)
		{
			m_OPLBytesBuffers.BuffersClear();
		}
	...
	private void CloseListenerPorts()
	{
		foreach (Port p in m_portsList)
		{
			if (p.m_par.connectRemote == 0)
			{
				...
				if (m_NNBase.m_isLogging)
				{
					m_NNBase.log("Closing listener port " + sport, isXml: false, "RTMOPL");
				}
				p.m_CommType.Notify(-1);
				if (m_NNBase.m_isLogging)
				{
					m_NNBase.log("Closed listener port " + sport, isXml: false, "RTMOPL");
				}
			}
		}
	}

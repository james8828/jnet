	...
	public class AsynchNetworkServer
	{
		public class ClientHandler
		{
			...
			public ClientHandler(AsynchNetworkServer parent, Socket socketForClient, string protocol, int portNum, string portType, string from_inst_id, bool logging)
			{
				...
				string protocol2;
				if ((protocol2 = m_protocol) != null && protocol2 == "DML")
				{
		...
		public void RemoveFromList(ClientHandler him)
		{
			...
			{
				m_parent.m_NNBase.log("Client disconnected from " + m_portNum + ":" + sh, isXml: false, "RTMOPL");
				string ThreadCount = "Thread Count after removing client handler:" + Process.GetCurrentProcess().Threads.Count;
				m_parent.m_NNBase.log(ThreadCount, isXml: false, "RTMOPL");
			}
		}
		...
		public bool Notify(int cd)
		{
			...
			case -1:
				...
				if (m_tcpListener != null)
				{
					...
					if (m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount2, isXml: false, "RTMOPL");
					}
				}
				break;
			}
			...
			foreach (ClientHandler h in handlers)
			{
				if (h.m_theProtocol != null && !h.m_theProtocol.m_isShutDown && !h.m_theProtocol.m_isShuttingDown)
				{
					...
					if (cd == -1 && m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount3, isXml: false, "RTMOPL");
					}
					...
					if (cd == -1 && m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount4, isXml: false, "RTMOPL");
					}
				}
			}
			handlers.Clear();
			if (cd == -1)
			{
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Clearing client handler list", isXml: false, "RTMOPL");
				}
				lock (m_handlers)
				{
					m_handlers.Clear();
				...
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Cleared client handler list", isXml: false, "RTMOPL");
				}
				if (m_socketForClient != null)
				{
					if (m_parent.m_NNBase.m_isLogging)
					{
						m_parent.m_NNBase.log("Closing socket for client", isXml: false, "RTMOPL");
					}
					m_socketForClient.Close();
					if (m_parent.m_NNBase.m_isLogging)
					{
						m_parent.m_NNBase.log("Closed socket for client", isXml: false, "RTMOPL");
					}
				}
				if (m_parent.m_NNBase.m_isLogging)
				{
					string ThreadCount5 = "Thread Count before stopping listener thread:" + Process.GetCurrentProcess().Threads.Count;
					m_parent.m_NNBase.log(ThreadCount5, isXml: false, "RTMOPL");
				}
				myListener.Abort();
				myListener.Join();
				if (m_parent.m_NNBase.m_isLogging)
				{
					...
					m_parent.m_NNBase.log(ThreadCount6, isXml: false, "RTMOPL");
				}
			}
			return false;
		}
		...
		public void theListener()
		{
			...
			try
			{
				...
				while (!m_parent.m_bShuttingDown && !m_ShuttingPort)
				{
					try
					{
						if (m_handlers.Count < MaxConnections)
						{
							...
							if (m_socketForClient.Connected)
							{
								if (m_parent.m_NNBase.m_isLogging)
								{
									m_parent.m_NNBase.log("Client connected on " + m_portNum + ":" + m_socketForClient.Handle.ToString(), isXml: false, "RTMOPL");
								}
					...
					catch
					{
						...
						{
							m_parent.m_NNBase.log("Listener port spawn exception", isXml: false, "RTMOPL");
						}
					}
				}
			}
			catch (InvalidOperationException ex)
			{
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Listener port accept exception: " + ex.Message, isXml: false, "RTMOPL");
				}
			}
		}
	}
		...
		public AsynchNetworkClient(PortParams par)
		{
			try
			{
				...
				if (serverName != null && serverName.Length > 0)
				{
					...
					{
						par.parent.m_NNBase.log("Connecting to " + remoteport, isXml: false, "RTMOPL");
					}
				...
				else
				{
					...
					if (serverName != null && serverName.Length > 0 && serverName != sipa)
					{
						...
						if (par.parent.m_NNBase.m_isLogging)
						{
							par.parent.m_NNBase.log("Connecting to " + remoteport2, isXml: false, "RTMOPL");
						}
						tcpSocket = new TcpClient(serverName, par.remotePort);
					}
					else
					{
						...
						if (par.parent.m_NNBase.m_isLogging)
						{
							par.parent.m_NNBase.log("Connecting to " + remoteport3, isXml: false, "RTMOPL");
						}
				...
				string protocol;
				if ((protocol = par.protocol) != null && protocol == "DML")
				{
			...
			catch (Exception ex)
			{
				...
				par.parent.m_NNBase.ReportErrorDB(sError, "E", "attempting to connect to OPL server", "AsynchNetworkClient", ex.Message);
				par.parent.m_NNBase.SendEventReport("Connection Refused", "OPL Feed");
			}
		}

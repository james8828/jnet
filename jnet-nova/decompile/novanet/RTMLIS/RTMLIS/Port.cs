	...
	public class AsynchNetworkServer
	{
		public class ClientHandler
		{
			...
			public ClientHandler(AsynchNetworkServer parent, Socket socketForClient, string protocol, int portNum, string portType, string from_inst_id, bool logging)
			{
				...
				{
				case "DML":
					m_theProtocol = new DMLProtocol(ref networkStream, socket.Handle.ToInt32(), portType, logging, m_parent.m_parent, this, portNum);
					break;
				case "ASTM":
					m_theProtocol = new ASTMProtocol(ref networkStream, socket.Handle.ToInt32(), portType, logging, m_parent.m_parent, this, portNum);
					break;
				case "HL7":
					m_theProtocol = new HL7Protocol(ref networkStream, socket.Handle.ToInt32(), portType, logging, m_parent.m_parent, this, portNum);
					break;
				}
				if (m_theProtocol != null && logging)
				{
			...
			public void RemoveFromList(string caller)
			{
				...
				{
					m_parent.m_parent.m_NNBase.log("Attempt to remove client handler when handler count is zero", isXml: false, caller);
				}
			}
		}
		...
		public void RemoveFromList(ClientHandler him)
		{
			...
			{
				m_parent.m_NNBase.log("Client disconnected from " + m_portNum + ":" + sh, isXml: false, "RTMLIS");
				string ThreadCount = "Thread Count after removing client handler:" + Process.GetCurrentProcess().Threads.Count;
				m_parent.m_NNBase.log(ThreadCount, isXml: false, "RTMLIS");
			}
		}
		...
		public bool Notify(int cd)
		{
			...
			case -1:
				...
				if (m_tcpListener != null)
				{
					...
					if (m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount2, isXml: false, "RTMLIS");
					}
				}
				break;
			}
			...
			foreach (ClientHandler h in handlers)
			{
				if (h.m_theProtocol != null && !h.m_theProtocol.m_isShutDown && !h.m_theProtocol.m_isShuttingDown)
				{
					...
					if (cd == -1 && m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount3, isXml: false, "RTMLIS");
					}
					...
					if (cd == -1 && m_parent.m_NNBase.m_isLogging)
					{
						...
						m_parent.m_NNBase.log(ThreadCount4, isXml: false, "RTMLIS");
					}
				}
			}
			handlers.Clear();
			if (cd == -1)
			{
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Clearing client handler list", isXml: false, "RTMLIS");
				}
				lock (m_handlers)
				{
					m_handlers.Clear();
				...
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Cleared client handler list", isXml: false, "RTMLIS");
				}
				if (m_socketForClient != null)
				{
					if (m_parent.m_NNBase.m_isLogging)
					{
						m_parent.m_NNBase.log("Closing socket for client", isXml: false, "RTMLIS");
					}
					m_socketForClient.Close();
					if (m_parent.m_NNBase.m_isLogging)
					{
						m_parent.m_NNBase.log("Closed socket for client", isXml: false, "RTMLIS");
					}
				}
				if (m_parent.m_NNBase.m_isLogging)
				{
					string ThreadCount5 = "Thread Count before stopping listener thread:" + Process.GetCurrentProcess().Threads.Count;
					m_parent.m_NNBase.log(ThreadCount5, isXml: false, "RTMLIS");
				}
				myListener.Abort();
				myListener.Join();
				if (m_parent.m_NNBase.m_isLogging)
				{
					...
					m_parent.m_NNBase.log(ThreadCount6, isXml: false, "RTMLIS");
				}
			}
			return false;
		}
		...
		public void theListener()
		{
			...
			try
			{
				...
				while (!m_parent.m_bShuttingDown && !m_ShuttingPort)
				{
					try
					{
						if (m_handlers.Count < MaxConnections)
						{
							...
							if (m_socketForClient.Connected)
							{
								if (m_parent.m_NNBase.m_isLogging)
								{
									m_parent.m_NNBase.log("Client connected on " + m_portNum + ":" + m_socketForClient.Handle.ToString(), isXml: false, "RTMLIS");
								}
					...
					catch
					{
						...
						{
							m_parent.m_NNBase.log("Listener port spawn exception", isXml: false, "RTMLIS");
						}
					}
				}
			}
			catch (InvalidOperationException ex)
			{
				if (m_parent.m_NNBase.m_isLogging)
				{
					m_parent.m_NNBase.log("Listener port accept exception: " + ex.Message, isXml: false, "RTMLIS");
				}
			}
		}
	}
	...
	public class AsynchNetworkClient
	{
		...
		public AsynchNetworkClient(PortParams par)
		{
			try
			{
				...
				string serverName = par.remoteHostName;
				TcpClient tcpSocket;
				if (serverName != null && serverName.Length > 0)
				{
					...
					{
						par.parent.m_NNBase.log("Connecting to " + remoteport, isXml: false, "RTMLIS");
					}
					tcpSocket = new TcpClient(serverName, par.remotePort);
				}
				else
				{
					IPAddress ipa = new IPAddress(par.ipAddress);
					...
					if (serverName != null && serverName.Length > 0 && serverName != sipa)
					{
						...
						if (par.parent.m_NNBase.m_isLogging)
						{
							par.parent.m_NNBase.log("Connecting to " + remoteport2, isXml: false, "RTMLIS");
						}
						tcpSocket = new TcpClient(serverName, par.remotePort);
					}
					else
					{
						...
						if (par.parent.m_NNBase.m_isLogging)
						{
							par.parent.m_NNBase.log("Connecting to " + remoteport3, isXml: false, "RTMLIS");
						}
				...
				{
				case "DML":
					m_theProtocol = new DMLProtocol(ref streamToServer, par.remotePort, par.portType, par.do_logging, par.parent, null, par.portNum);
					break;
				case "ASTM":
					m_theProtocol = new ASTMProtocol(ref streamToServer, par.remotePort, par.portType, par.do_logging, par.parent, null, par.portNum);
					break;
				case "HL7":
					m_theProtocol = new HL7Protocol(ref streamToServer, par.remotePort, par.portType, par.do_logging, par.parent, null, par.portNum);
					break;
				}
				if (m_theProtocol != null && par.do_logging)
				{
					m_theProtocol.ProcessNotify(1);
				}
				if (m_theProtocol != null)
			...
			catch (Exception ex)
			{
				...
				par.parent.m_NNBase.ReportErrorDB(sError, "E", "attempting to connect to LIS server", "AsynchNetworkClient", ex.Message);
				par.parent.m_NNBase.SendEventReport("Connection Refused", "LIS Feed");
			}
		}
